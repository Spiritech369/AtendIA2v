...
class _FakeNLU:
    ...
    async def classify(
        self, *, text: str, current_stage: str,
        ...
        return (
            ...
            UsageMetadata(
                model="stub", tokens_in=10, tokens_out=5,
...
async def test_runner_dispatches_RETENTION_on_gracias(db_session) -> None:
    ...
    tid, _, conv_id = await _seed(db_session, "test_t25_retention",
                                  _pipeline_with_rules(rules))
    ...
    trace = await runner.run_turn(
        ...
        inbound=_msg(conv_id, tid, "gracias por la info"), turn_number=1,
    )
    await db_session.commit()
...
async def test_runner_dispatches_OBSTACLE_on_manana_with_accent(db_session) -> None:
    ...
    rules = [
        {"id": "block", "trigger": {
            "type": "keyword_in_text", "list": ["mañana", "luego"]},
            "mode": "OBSTACLE"},
        {"id": "always", "trigger": {"type": "always"}, "mode": "SUPPORT"},
    ]
    tid, _, conv_id = await _seed(db_session, "test_t25_obstacle",
                                  _pipeline_with_rules(rules))
    composer = _RecordingComposer()
    runner = ConversationRunner(db_session, _FakeNLU(), composer)
    trace = await runner.run_turn(
        conversation_id=conv_id, tenant_id=tid,
        inbound=_msg(conv_id, tid, "te lo paso mañana"), turn_number=1,
    )
    await db_session.commit()
...
async def test_runner_dispatches_PLAN_when_plan_credito_missing(db_session) -> None:
    ...
    rules = [
        {"id": "plan", "trigger": {
            "type": "field_missing", "field": "plan_credito"},
            "mode": "PLAN"},
        {"id": "always", "trigger": {"type": "always"}, "mode": "SUPPORT"},
    ]
    tid, _, conv_id = await _seed(db_session, "test_t25_plan",
                                  _pipeline_with_rules(rules))
    composer = _RecordingComposer()
    runner = ConversationRunner(db_session, _FakeNLU(Intent.GREETING), composer)
    trace = await runner.run_turn(
        conversation_id=conv_id, tenant_id=tid,
        inbound=_msg(conv_id, tid, "hola"), turn_number=1,
    )
    await db_session.commit()
...
async def test_runner_dispatches_SALES_when_plan_present_and_intent_buy(
    ...
    tid, _, conv_id = await _seed(db_session, "test_t25_sales",
                                  _pipeline_with_rules(rules))
    # Pre-seed extracted_data with plan_credito so the field_present trigger fires.
    await db_session.execute(
        text("UPDATE conversation_state SET extracted_data = :ed\\:\\:jsonb "
             "WHERE conversation_id = :cid"),
        {
            "ed": json.dumps({
                "plan_credito": {
                    "value": "10%", "confidence": 1.0, "source_turn": 0,
                },
            }),
            "cid": conv_id,
        },
    ...
    trace = await runner.run_turn(
        conversation_id=conv_id, tenant_id=tid,
        inbound=_msg(conv_id, tid, "cuánto cuesta la 150Z"), turn_number=2,
    )
    await db_session.commit()
...
async def test_runner_dispatches_SUPPORT_via_always_fallback(db_session) -> None:
    ...
    tid, _, conv_id = await _seed(db_session, "test_t25_support",
                                  _pipeline_with_rules(rules))
    composer = _RecordingComposer()
    runner = ConversationRunner(db_session, _FakeNLU(Intent.ASK_INFO), composer)
    trace = await runner.run_turn(
        conversation_id=conv_id, tenant_id=tid,
        inbound=_msg(conv_id, tid, "qué tipos de plan tienen"), turn_number=1,
    )
    await db_session.commit()
...
async def test_runner_dispatches_DOC_on_pending_confirmation_then_sales(
    ...
    """
    rules = [
        {"id": "confirm", "trigger": {"type": "pending_confirmation"},
            "mode": "PLAN"},
        {"id": "always", "trigger": {"type": "always"}, "mode": "SUPPORT"},
    ]
    tid, _, conv_id = await _seed(db_session, "test_t25_pending",
                                  _pipeline_with_rules(rules))
    ...
    trace = await runner.run_turn(
        conversation_id=conv_id, tenant_id=tid,
        inbound=_msg(conv_id, tid, "respuesta libre"), turn_number=1,
    )
    await db_session.commit()
    assert trace.flow_mode == "PLAN"
    assert composer.last_input.flow_mode == FlowMode.PLAN
