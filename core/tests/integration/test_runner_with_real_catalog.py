        ...
        async with SessionLocal() as session:
            ...
            session.add(TenantCatalogItem(
                ...
                attrs={
                    ...
                    "planes_credito": {
                        "plan_10": {
                            "enganche": 3140, "pago_quincenal": 1247, "quincenas": 72,
                        },
                    },
                },
                active=True,
            ...
            await session.execute(
                text("INSERT INTO conversations (id, tenant_id, customer_id, current_stage) "
                     "VALUES (:cid, :t, :c, 'greeting')"),
                {"cid": cid, "t": tid, "c": cust_id},
            )
            await session.execute(
                text("INSERT INTO conversation_state (conversation_id, "
...
def test_runner_dispatches_real_quote_via_alias(adventure_catalog) -> None:
    ...
    assert payload.get("status") == "ok", (
        f"Expected status='ok' from real quote(), got: {payload}"
    )
