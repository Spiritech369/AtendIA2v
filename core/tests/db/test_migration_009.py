...
async def test_followups_and_handoffs_tables_exist():
    ...
    async with engine.connect() as conn:
        def _check(sync_conn):
            ...
            assert {"id", "conversation_id", "tenant_id", "run_at", "template_id",
                    "status", "attempts", "last_error", "created_at"} <= f_cols
            h_cols = {c["name"] for c in insp.get_columns("human_handoffs")}
            assert {"id", "conversation_id", "tenant_id", "reason", "assigned_user_id",
                    "status", "requested_at", "resolved_at"} <= h_cols
...
async def test_followup_lifecycle():
    ...
    async with engine.begin() as conn:
        ...
        cid = (await conn.execute(
            text("INSERT INTO customers (tenant_id, phone_e164) VALUES (:t, '+5215555550021') RETURNING id"),
            {"t": tid},
        )).scalar()
        ...
        await conn.execute(
            text("INSERT INTO followups_scheduled (conversation_id, tenant_id, run_at) "
                 "VALUES (:c, :t, :r)"),
...
async def test_handoff_default_status_open():
    ...
    async with engine.begin() as conn:
        ...
        cid = (await conn.execute(
            text("INSERT INTO customers (tenant_id, phone_e164) VALUES (:t, '+5215555550022') RETURNING id"),
            {"t": tid},
        )).scalar()
        conv_id = (await conn.execute(
            text("INSERT INTO conversations (tenant_id, customer_id) VALUES (:t, :c) RETURNING id"),
        ...
        await conn.execute(
            text("INSERT INTO human_handoffs (conversation_id, tenant_id, reason) "
