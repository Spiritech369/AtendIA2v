...
async def test_turn_traces_and_tool_calls_tables_exist():
    ...
    async with engine.connect() as conn:
        def _check(sync_conn):
            ...
            assert {"id", "conversation_id", "tenant_id", "turn_number",
                    "inbound_message_id", "inbound_text",
                    "nlu_input", "nlu_output", "nlu_model",
                    "nlu_tokens_in", "nlu_tokens_out", "nlu_cost_usd", "nlu_latency_ms",
                    "state_before", "state_after", "stage_transition",
                    "composer_input", "composer_output", "composer_model",
                    "composer_tokens_in", "composer_tokens_out", "composer_cost_usd", "composer_latency_ms",
                    "outbound_messages", "total_cost_usd", "total_latency_ms",
                    "errors", "created_at"} <= tt_cols
            tc_cols = {c["name"] for c in insp.get_columns("tool_calls")}
            assert {"id", "turn_trace_id", "tool_name", "input", "output",
                    "latency_ms", "error", "called_at"} <= tc_cols
...
async def test_turn_trace_with_tool_calls_lifecycle():
    ...
    async with engine.begin() as conn:
        tid = (await conn.execute(
            text("INSERT INTO tenants (name) VALUES ('test_t18') RETURNING id")
        )).scalar()
        cid = (await conn.execute(
            text("INSERT INTO customers (tenant_id, phone_e164) VALUES (:t, '+5215555550018') RETURNING id"),
            {"t": tid},
        )).scalar()
