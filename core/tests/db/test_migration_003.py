...
async def test_conversation_with_state_lifecycle():
    ...
    async with engine.begin() as conn:
        tid = (
            await conn.execute(text("INSERT INTO tenants (name) VALUES ('test_t15') RETURNING id"))
        ).scalar()
        cid = (
            await conn.execute(
                text(
                    "INSERT INTO customers (tenant_id, phone_e164) VALUES (:t, '+5215555550015') RETURNING id"
                ),
                {"t": tid},
            )
        ).scalar()
        ...
        rem_state = (
            await conn.execute(
                text("SELECT COUNT(*) FROM conversation_state WHERE conversation_id = :c"),
                {"c": conv_id},
            )
        ).scalar()
        assert rem_conv == 0
