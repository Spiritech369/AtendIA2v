...
HEALTH_COLUMNS = [
    "id", "tenant_id", "snapshot_at", "score",
    "score_components", "main_risks", "suggested_actions",
    "per_collection_scores",
]
AGENT_PERM_COLUMNS = [
    "id", "tenant_id", "agent",
    "allowed_source_types", "allowed_collection_slugs",
    "min_score", "can_quote_prices", "can_quote_stock",
    "required_customer_fields", "escalate_on_conflict",
    "fallback_message", "updated_at", "updated_by",
]
PRIORITY_RULE_COLUMNS = [
    "id", "tenant_id", "agent", "source_type", "priority",
    "minimum_score", "allow_synthesis", "allow_direct_answer",
    "escalation_required_when_conflict", "updated_at",
]
SAFE_ANSWER_COLUMNS = [
    "tenant_id", "min_score_to_answer",
    "escalate_on_conflict", "block_invented_prices", "block_invented_stock",
    "risky_phrases", "default_fallback_message",
    ...
    try:
        async with engine.begin() as conn:
            tid = (await conn.execute(text(
                "INSERT INTO tenants (name) VALUES ('test_kb_t036') RETURNING id"
            ))).scalar()
...
async def test_kb_safe_answer_settings_default_fallback_is_spanish():
    ...
    try:
        async with engine.begin() as conn:
            tid = (await conn.execute(text(
                "INSERT INTO tenants (name) VALUES ('test_kb_t036b') RETURNING id"
            ))).scalar()
            await conn.execute(text(
                "INSERT INTO kb_safe_answer_settings (tenant_id) VALUES (:t)"
            ), {"t": tid})
