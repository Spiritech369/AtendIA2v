...
async def test_events_insert_and_jsonb_payload():
    ...
    async with engine.begin() as conn:
        tid = (
            await conn.execute(text("INSERT INTO tenants (name) VALUES ('test_t17') RETURNING id"))
        ).scalar()
        cid = (
            await conn.execute(
                text(
                    "INSERT INTO customers (tenant_id, phone_e164) VALUES (:t, '+5215555550017') RETURNING id"
                ),
                {"t": tid},
            )
        ).scalar()
