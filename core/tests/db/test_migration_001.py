...
async def test_tenants_can_insert_and_query():
    ...
    async with engine.begin() as conn:
        await conn.execute(text("INSERT INTO tenants (name) VALUES ('test_tenant_001')"))
        result = await conn.execute(text("SELECT name FROM tenants WHERE name = 'test_tenant_001'"))
        row = result.fetchone()
