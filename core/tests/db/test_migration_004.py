...
async def test_messages_table_structure():
    ...
    async with engine.connect() as conn:
        def _check(sync_conn):
            ...
            assert {"id", "conversation_id", "tenant_id", "direction", "text",
                    "channel_message_id", "delivery_status", "metadata_json",
                    "sent_at", "created_at"} <= cols
...
async def test_messages_check_constraint_rejects_invalid_direction():
    ...
    async with engine.begin() as conn:
        tid = (await conn.execute(
            text("INSERT INTO tenants (name) VALUES ('test_t16') RETURNING id")
        )).scalar()
        cid = (await conn.execute(
            text("INSERT INTO customers (tenant_id, phone_e164) VALUES (:t, '+5215555550016') RETURNING id"),
            {"t": tid},
        )).scalar()
        ...
        with pytest.raises(IntegrityError):
            await conn.execute(
                text("INSERT INTO messages (conversation_id, tenant_id, direction, text, sent_at) "
