...
async def seeded_tenant(db_session):
    ...
    async with eng.begin() as conn:
        tid = (await conn.execute(text(
            "INSERT INTO tenants (name) VALUES ('test_kb_retriever') RETURNING id"
        ))).scalar()
    await eng.dispose()
...
async def _collection_id(session: AsyncSession, tenant_id: UUID, slug: str) -> UUID:
    return (
        await session.execute(text(
            "SELECT id FROM kb_collections WHERE tenant_id=:t AND slug=:s"
        ), {"t": tenant_id, "s": slug})
    ).scalar_one()
...
async def _insert_faq(
    ...
    *,
    ...
    fid = (await session.execute(text(
        ...
        f"RETURNING id"
    ), {"t": tenant_id, "q": question, "a": answer, "e": embedding_lit, "s": status, "c": coll_id})).scalar_one()
...
async def _insert_catalog(
    ...
    *,
    ...
    cid = (await session.execute(text(
        ...
        "RETURNING id"
    ), {"t": tenant_id, "s": sku, "n": name, "e": embedding_lit, "c": coll_id})).scalar_one()
    await session.commit()
    return cid
...
async def test_recepcionista_cannot_retrieve_catalog(db_session, seeded_tenant):
    ...
    await _insert_catalog(
        db_session, tid,
        sku="SKU-1", name="Modelo Dinamo U5", collection_slug="catalogo",
    )
    result = await retrieve(
        db_session, tid,
...
async def test_sales_agent_does_retrieve_catalog(db_session, seeded_tenant):
    ...
    await _insert_catalog(
        db_session, tid,
        sku="SKU-2", name="Modelo Dinamo X9", collection_slug="catalogo",
    )
    result = await retrieve(
        db_session, tid,
