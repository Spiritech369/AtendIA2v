...
async def test_alias_keyword_match(db_session) -> None:
    ...
    await _seed_item(db_session, tid, sku="adventure-150", name="Adventure 150",
                     alias=["adventure", "elite"], embedding=_emb(0))
...
async def test_alias_match_is_case_insensitive(db_session) -> None:
    ...
    await _seed_item(db_session, tid, sku="adv-1", name="Adventure",
                     alias=["adventure"], embedding=_emb(0))
    await db_session.commit()
    try:
        result = await search_catalog(
            session=db_session, tenant_id=tid, query="ADVENTURE", embedding=None,
...
async def test_falls_back_to_no_data_when_no_alias_and_no_embedding(db_session) -> None:
    ...
    await _seed_item(db_session, tid, sku="x-1", name="X",
                     alias=["other"], embedding=_emb(0))
    await db_session.commit()
    try:
        result = await search_catalog(
...
async def test_respects_limit(db_session) -> None:
    ...
    for i in range(10):
        await _seed_item(db_session, tid, sku=f"sku-{i}", name=f"Name {i}",
                         alias=["shared"], embedding=_emb(i))
    await db_session.commit()
    try:
        result = await search_catalog(
            session=db_session, tenant_id=tid, query="shared", embedding=None, limit=3,
        )
        assert isinstance(result, list)
        assert len(result) == 3
    finally:
...
async def test_filters_inactive(db_session) -> None:
    ...
    await _seed_item(db_session, tid, sku="old-1", name="Old", category="Motoneta",
                     alias=["old"], embedding=None, active=False)
    await db_session.commit()
    try:
        result = await search_catalog(
...
async def test_tenant_isolation(db_session) -> None:
    ...
    await _seed_item(db_session, tid_b, sku="leaked", name="Leak",
                     alias=["unique"], embedding=_emb(0))
    await db_session.commit()
    try:
        result = await search_catalog(
            session=db_session, tenant_id=tid_a, query="unique", embedding=_emb(0),
        )
        assert isinstance(result, ToolNoDataResult)
    finally:
