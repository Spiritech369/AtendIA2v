...
def _prompt(context: str = "<fuente>x</fuente>") -> PromptInput:
    return PromptInput(
        system="sys", user="¿enganche?", context=context,
        response_instructions="responde",
        model="gpt-4o-mini", max_tokens=200, temperature=0.0,
    )


def _chunk(score: float, text: str = "Enganche desde 10%") -> RetrievedChunk:
    return RetrievedChunk(
        source_type="faq", source_id=uuid4(), text=text, score=score, collection="credito",
    )
...
async def test_conflicts_short_circuit_to_escalate():
    retrieval = RetrievalResult(
        chunks=[_chunk(0.95)],
        conflicts=[{"detection_type": "price_mismatch", "title": "x", "severity": "high",
                    "entity_a_type": "faq", "entity_a_id": "a", "entity_a_excerpt": "",
                    "entity_b_type": "faq", "entity_b_id": "b", "entity_b_excerpt": ""}],
        total_candidates=1,
    )
    out = await synthesize(retrieval, _prompt(), SafeAnswerSettings(escalate_on_conflict=True), "sales_agent", MockProvider())
...
async def test_no_chunks_escalates():
    ...
    out = await synthesize(retrieval, _prompt(), SafeAnswerSettings(), "duda_general", MockProvider())
    assert out.action == "escalate"
    assert out.mode == "empty"
...
async def test_top_score_below_floor_escalates():
    ...
    out = await synthesize(retrieval, _prompt(), SafeAnswerSettings(min_score_to_answer=0.7), "duda_general", MockProvider())
    assert out.action == "escalate"
    assert out.mode == "empty"
...
async def test_below_70_with_risks_falls_to_low_escalate():
    ...
    out = await synthesize(retrieval, _prompt(), SafeAnswerSettings(min_score_to_answer=0.70), "duda_general", MockProvider())
...
async def test_high_score_with_conflict_still_demoted_to_low():
    ...
    retrieval = RetrievalResult(
        chunks=[_chunk(0.95)],
        conflicts=[{"detection_type": "x", "title": "t", "severity": "low",
                    "entity_a_type": "faq", "entity_a_id": "a", "entity_a_excerpt": "",
                    "entity_b_type": "faq", "entity_b_id": "b", "entity_b_excerpt": ""}],
        total_candidates=1,
    )
    out = await synthesize(retrieval, _prompt(), SafeAnswerSettings(escalate_on_conflict=False), "duda_general", MockProvider())
    assert out.confidence != "high"
