...
async def tenant_id():
    ...
    async with engine.begin() as conn:
        tid = (await conn.execute(text(
            "INSERT INTO tenants (name) VALUES ('test_kb_seed') RETURNING id"
        ))).scalar()
    try:
        yield tid
    finally:
...
async def test_seed_is_idempotent(db_session, tenant_id):
    ...
    n_perms = (await db_session.execute(text(
...
async def test_seed_safe_answer_includes_risky_phrases(db_session, tenant_id):
    ...
    raw = (await db_session.execute(text(
        "SELECT risky_phrases FROM kb_safe_answer_settings WHERE tenant_id=:t"
    ), {"t": tenant_id})).scalar_one()
