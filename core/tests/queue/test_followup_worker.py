...
async def _seed_tenant_conv(db_session, *, followups_enabled: bool = True) -> tuple:
    ...
    await db_session.execute(
        text("INSERT INTO conversation_state (conversation_id, extracted_data) "
             "VALUES (:c, :ed\\:\\:jsonb)"),
...
async def _insert_due_followup(
    db_session, *, conv, tid, kind: str = "3h_silence",
    minutes_overdue: int = 1, status: str = "pending",
    cancelled: bool = False, enqueued: bool = False,
) -> None:
    await db_session.execute(
...
async def test_poll_skips_when_no_due_rows(db_session, monkeypatch, fake_arq_redis):
    ...
    monkeypatch.setattr(
        "atendia.queue.followup_worker._quiet_hours_active", lambda _now: False,
    )
...
async def test_poll_skips_during_quiet_hours(db_session, monkeypatch, fake_arq_redis):
    monkeypatch.setattr(
        "atendia.queue.followup_worker._quiet_hours_active", lambda _now: True,
    )
    result = await poll_followups({"redis": fake_arq_redis})
...
async def test_poll_fires_due_3h_and_marks_sent(
    db_session, monkeypatch, fake_arq_redis,
):
    monkeypatch.setattr(
        "atendia.queue.followup_worker._quiet_hours_active", lambda _now: False,
    )
    ...
    status = (await db_session.execute(
        text("SELECT status FROM followups_scheduled "
    ...
    cnt = (await db_session.execute(
        text("SELECT followups_sent_count FROM conversation_state "
...
async def test_poll_3h_fire_reschedules_pending_12h(
    ...
    cancelled_old = (await db_session.execute(
    ...
    new_pending = (await db_session.execute(
        text("SELECT COUNT(*), MIN(run_at) FROM followups_scheduled "
             "WHERE conversation_id = :c AND kind = '12h_silence' "
             "  AND status = 'pending'"),
        {"c": conv},
    )).fetchone()
...
async def test_poll_marks_failed_on_missing_phone(
    ...
    async def _fake_no_phone(_session, _conversation_id):
        return None
    monkeypatch.setattr(
        "atendia.queue.followup_worker._load_recipient_phone", _fake_no_phone,
    )
    ...
    status = (await db_session.execute(
        text("SELECT status, last_error FROM followups_scheduled "
             "WHERE conversation_id = :c"),
        {"c": conv},
    )).fetchone()
...
async def test_poll_caps_at_50_per_tick(
    ...
    unfired_3h = (await db_session.execute(
        text("SELECT COUNT(*) FROM followups_scheduled "
             "WHERE conversation_id = :c AND kind = '3h_silence' "
             "  AND status = 'pending'"),
        {"c": conv},
    ...
    pending_12h = (await db_session.execute(
        text("SELECT COUNT(*) FROM followups_scheduled "
             "WHERE conversation_id = :c AND kind = '12h_silence' "
             "  AND status = 'pending'"),
        {"c": conv},
