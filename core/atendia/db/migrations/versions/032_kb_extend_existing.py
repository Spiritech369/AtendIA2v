...
def _add_status(table: str) -> None:
    op.add_column(
        table, sa.Column("status", sa.String(20), nullable=False, server_default="published")
    )


def _add_shared_metadata_minus_status(table: str) -> None:
    op.add_column(
        table, sa.Column("visibility", sa.String(20), nullable=False, server_default="agents")
    )
    ...
    op.add_column(
        table,
        sa.Column(
            "collection_id", sa.UUID(), sa.ForeignKey("kb_collections.id", ondelete="SET NULL")
        ),
    )
    op.add_column(
        table, sa.Column("language", sa.String(8), nullable=False, server_default="es-MX")
    )
...
def upgrade() -> None:
    ...
    op.add_column(
        "tenant_catalogs",
        sa.Column("stock_status", sa.String(20), nullable=False, server_default="unknown"),
    )
    ...
    op.execute(
        "UPDATE knowledge_documents SET embedded_chunk_count=fragment_count WHERE status='ready'"
    )

    # knowledge_chunks additions
    op.add_column(
        "knowledge_chunks",
        sa.Column("chunk_status", sa.String(20), nullable=False, server_default="embedded"),
    )
    op.add_column(
        "knowledge_chunks",
        sa.Column("marked_critical", sa.Boolean(), nullable=False, server_default=sa.text("false")),
    )
