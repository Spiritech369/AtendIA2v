...
export function OverviewSection({ trace: t }: { trace: TurnTraceDetail }) {
  ...
  return (
    <div className="space-y-2 p-3">
      ...
      {t.inbound_text && (
        <div className="rounded-md bg-muted p-2 text-xs italic line-clamp-2">{t.inbound_text}</div>
      )}
    </div>
  );
}
    ...
    <div className="space-y-2 p-3">
      ...
      {errors.length === 0 ? (
        ...
        <div className="space-y-1">
          {errors.map((err, i) => {
            const serialized = typeof err === "string" ? err : JSON.stringify(err, null, 2);
            return (
              <div
                // biome-ignore lint/suspicious/noArrayIndexKey: errors come from immutable trace JSON and never reorder.
