...
| Frontend `pnpm exec vitest run` | 21 files, 97 tests passed | Incluye `auth-guards` (4), `RoleGate` (4), `turnStory` (10), `FlowModeBadge` (4), `TurnStoryView` (8), `useCustomerAttrs` (4), `EditableDetailRow` (8), `AddCustomAttrDialog` (8), `menu-config` (6), `useNavBadges` (2), `sidebar-store` (5), `SidebarItem` (9), `SidebarGroup` (4), `useFieldSuggestions` (3), `FieldSuggestionsPanel` (4). |
| Backend `pytest core/tests/api/test_navigation_badges.py` | 3/3 passed | Counts agregados verifican happy path, tenant isolation y auth required. |
| Backend `pytest core/tests/api/test_baileys_routes.py` | 9/9 passed | status/connect/disconnect RBAC + QR pass-through + preference + internal token + inbound creation + dedup. |
| Backend `pytest core/tests/queue/test_outbound_dispatcher_baileys.py` | 6/6 passed | Matriz de routing Baileys vs Meta (4 combos enabled/prefer/status + no-row + template rejection). |
| Backend `pytest core/tests/runner/test_field_extraction_mapping.py` | 11/11 passed | `decide_action` pure: AUTO/SUGGEST/SKIP/NOOP por la tabla de reglas del design doc. |
| Backend `pytest core/tests/runner/test_ai_extraction_service.py` | 6/6 passed | Cascade NLU → customer.attrs / field_suggestions contra DB real. |
| Backend `pytest core/tests/api/test_field_suggestions_routes.py` | 5/5 passed | List/accept/reject + idempotencia 409 + tenant isolation. |
...
|---|---|---|---|---:|---|---|
| App shell y navegacion | Envolver la app autenticada, mostrar tenant/rol, menu lateral con grupos + badges dinamicos, estado WhatsApp y notificaciones. | `core/atendia/main.py`, `core/atendia/api/notifications_routes.py`, `core/atendia/api/channel_status_routes.py`, `core/atendia/api/navigation_routes.py` (nuevo) | `frontend/src/components/AppShell.tsx`, `frontend/src/components/sidebar/*` (AppSidebar, SidebarHeader, SidebarGroup, SidebarItem, SidebarBadge, SidebarFooter), `frontend/src/features/navigation/*` (menu-config, hooks, types), `frontend/src/stores/sidebar-store.ts` | 92 | Sidebar reagrupado en 6 secciones, filtrado RBAC declarativo, badges polling 30s (`/navigation/badges`), modo compacto + persistencia localStorage, aria-current/aria-expanded. Backend agrega 6 conteos (conversations_open, handoffs_open + overdue, appointments_today, ai_debug_warnings, unread_notifications). 24 tests nuevos (3 backend + 21 frontend). | No hay tenant switcher (modelo de membership es 1:1 por usuario); no hay mobile drawer todavia; el estilo especial de handoffs sigue hardcoded por ruta. |
...
| Integrations / WhatsApp status | Mostrar salud de canal Meta/WhatsApp + canal Baileys QR alterno + proveedor IA. | `core/atendia/api/integrations_routes.py`, `core/atendia/api/channel_status_routes.py`, `core/atendia/api/baileys_routes.py` (nuevo), `core/atendia/channels/meta_cloud_api.py`, `core/atendia/integrations/baileys_client.py` (nuevo), `core/atendia/queue/circuit_breaker.py`, `core/atendia/queue/worker.py` (routing), `core/baileys-bridge/` (sidecar Node.js, nuevo) | `frontend/src/features/config/components/IntegrationsTab.tsx`, `frontend/src/features/config/components/BaileysCard.tsx` (nuevo), `frontend/src/features/config/baileys-api.ts` (nuevo), `frontend/src/components/WhatsAppStatusBadge.tsx` | 84 | Card Meta Business + Card Baileys QR con 5 estados (disconnected/connecting/qr_pending/connected/error), polling adaptativo, persistencia de sesión vía volumen Docker, switch "preferir Baileys sobre Meta" para outbound, sidecar Node con auto-reconnect, inbound bidireccional al backend con X-Internal-Token. 15 tests nuevos backend (9 routes + 6 dispatcher). | Sin campañas masivas / stealth / proxy / multimedia (Phase 2). Inbound Baileys persiste pero NO ejecuta el runner conversacional aún (Phase 2 — operator responde manual). Sin tenant switcher / multi-sesión por tenant. Riesgo de baneo Meta es responsabilidad del usuario; documentado en design doc. |
...
5. ~~Reagrupar sidebar plano en grupos colapsables con badges dinámicos y RBAC declarativo.~~ **Completado 2026-05-13** — 6 grupos, `menu-config.ts`, endpoint nuevo `/api/v1/navigation/badges` con 6 counts, store Zustand con persist, modo compacto. Ver `docs/plans/2026-05-13-sidebar-redesign-design.md`.
6. ~~Canal WhatsApp alterno por QR (Baileys) al lado del Meta Business API.~~ **Completado 2026-05-13** — Sidecar Node.js `core/baileys-bridge/` con Baileys 7.x, 6 endpoints backend `/api/v1/integrations/baileys/*` + `/api/v1/internal/baileys/inbound` con X-Internal-Token, routing outbound en `worker.py` (Baileys vs Meta según preferencia), `BaileysCard.tsx` con 5 estados + QR PNG + auto-reconnect + volumen persistente. Ver `docs/plans/2026-05-13-baileys-integration-design.md`. Phase 2 pendiente: cablear inbound al `conversation_runner`, multimedia, campañas.
7. ~~Cablear extracción IA → campos del panel (auto-apply ≥0.85, sugerencias en banda 0.60-0.84, UI accept/reject).~~ **Completado 2026-05-13** — Migración 043 `field_suggestions`, `decide_action` pure logic + `apply_ai_extractions` service cableados en `conversation_runner` después del merge de entities. Endpoints `/api/v1/customers/:cid/field-suggestions` (list) + `/api/v1/field-suggestions/:sid/{accept,reject}`. `FieldSuggestionsPanel.tsx` arriba del grid en `ContactPanel`. 22 tests nuevos. Ver `docs/plans/2026-05-13-ai-field-extraction-design.md`.
