...
| Feature | Proposito | Logica | UI/UX | Score | Funciona | No funciona / riesgo |
|---|---|---|---|---:|---|---|
| Conversations inbox | Bandeja de conversaciones, detalle, mensajes, panel de contacto, intervencion humana, pausa/reanudar bot. | `core/atendia/api/conversations_routes.py`, `core/atendia/db/models/conversation.py`, `core/atendia/db/models/message.py`, `core/atendia/queue/force_summary_job.py` | `frontend/src/features/conversations/components/*`, `frontend/src/features/conversations/hooks/*`, `frontend/src/features/conversations/api.ts`, `frontend/src/routes/(auth)/index.tsx`, `frontend/src/routes/(auth)/conversations.$conversationId.tsx` | 88 | Listado, detalle, mensajes, mark-read, delete, intervene, resume-bot, realtime hooks. Tests API y frontend de lista existen. | El handoff doc previo marca gaps de paridad v1; DebugPanel rico y algunos refinamientos UX siguen pendientes. |
