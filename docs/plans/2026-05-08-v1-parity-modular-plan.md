...
- Knowledge Base — **hardened in sessions 2 + 3, expanded to B2 module in 2026-05-10 session**:
  ...
  - **2026-05-10 — KB module B2 partial**: 6 migrations (031-036) ship 11 new tables + extended FAQs/Catalog/Documents/Chunks; 10 SQLAlchemy models; full RAG pipeline (provider abstraction with OpenAI + Mock, agent-scoped retriever, prompt builder, answer synthesizer, conflict detector, risky-phrase detector); idempotent seed script (9 collections + 4 agent permissions + risky phrases + priority rules); 3 new endpoints (`/search`, `/test-query`, `/collections`); runbook at [`docs/runbooks/knowledge-base.md`](../runbooks/knowledge-base.md). ~70 new tests, all green. Design + plan in [`2026-05-10-knowledge-base-module-design.md`](2026-05-10-knowledge-base-module-design.md) + [`2026-05-10-knowledge-base-module-implementation.md`](2026-05-10-knowledge-base-module-implementation.md).
  - **Still pending**: HNSW partial-index per tenant (cross-tenant query leakage at scale, documented), frontend KB rebuild (8 tabs + PromptPreviewDrawer + ChunkEditorDrawer + BulkActionsBar + ~35 components), ~44 remaining endpoints (chunks/conflicts/unanswered/tests/versions/health/analytics/settings, FAQ/Catalog publish/archive, document stage triggers, catalog import), 5 worker jobs (detect_conflicts, compute_health_snapshot, expire_content, run_regression_suite, import_catalog_csv), real OpenAI E2E with operator sign-off.
