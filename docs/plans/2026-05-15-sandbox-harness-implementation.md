...
**Step 3: Implement** `run_sandbox_conversation(*, conversation_id, tenant_id, script: list[str], cost_cap_usd: Decimal | None, ...)` that loops `run_sandbox_turn` over the script, accumulates `total_cost_usd`, and raises `CostCapExceeded(partial=..., spent=...)` the moment the running total exceeds the cap. Add an `estimate_cost(...)` helper that returns `n_turns * avg_cost_per_turn` where `avg_cost_per_turn` is read from the tenant's recent `turn_traces` (fallback constant `Decimal("0.02")` when no history).
