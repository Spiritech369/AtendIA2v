...
**Decisión clave: usar `halfvec(3072)` en vez de `vector(3072)`**

`text-embedding-3-large` produce vectores de 3072 dimensiones. pgvector tiene
un límite duro de **2000 dims para HNSW** sobre el tipo `vector` estándar
(intentar crear el índice falla con `column cannot have more than 2000
dimensions for hnsw index`). El tipo `halfvec` (half-precision, 16-bit floats)
sube ese límite a 4000 dims y además ahorra ~50% del espacio en disco
(~6 KB/row vs ~12 KB/row), con <0.5% de pérdida de recall en top-k.

`halfvec_cosine_ops` es el operador equivalente a `vector_cosine_ops` para
HNSW; el operador `<=>` funciona idéntico.
...
Expected: columns include `embedding halfvec(3072)` and `category varchar(60)`. Index `ix_tenant_catalogs_embedding hnsw (embedding halfvec_cosine_ops)`.
...
**Impact downstream:** Tareas T6/T9/T10/T11/T12 deben usar `HALFVEC(3072)` (modelo SQLAlchemy) y construir vectores de query como `pgvector.HalfVector(values)` o castear en SQL con `::halfvec`. El operador de distancia `<=>` no cambia.
