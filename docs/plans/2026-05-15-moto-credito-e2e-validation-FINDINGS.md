...
## Task 6 — Workflow create+trigger+execute — ✅ PASS

Code: `tools/e2e/task6_workflow.py` (run via core venv; no writes under `core/`). Reuses `tools/e2e/e2e_setup.py::Client`. Connects to the dev DB (`postgresql+asyncpg://atendia:atendia@localhost:5433/atendia_v2`) for the committed seed + the runner turn + the DB-authoritative execution poll/cleanup.

### Workflow contract pinned (READ-ONLY from source — do NOT guess)
- `POST /api/v1/workflows` → `WorkflowBody` (`core/atendia/api/workflows_routes.py:35-54`): `name` (1..200), `trigger_type` (must be in `engine.TRIGGERS`), `trigger_config` dict, `definition` dict (structurally validated by `engine.validate_definition`, `core/atendia/workflows/engine.py:179-316`), `active` bool. `stage_entered` is a valid trigger (`engine.py:70`). The whole definition can be POSTed inline — no separate `/nodes` calls needed (used the inline path; `POST /{id}/nodes` `:1363-1391` appends one node at a time and is the alternative).
- Trigger match: `_trigger_matches` (`engine.py:643-669`) fires when `trigger_config {"to": X}` equals event `payload {"to": X}`. Set `trigger_config={"to":"papeleria_completa"}`.
- Node types from `engine.NODE_TYPES` (`engine.py:90-120`): used `trigger` → `assign_agent` (`_node_assign_agent` `:1024-1060`, sets `conversation.assigned_agent_id`) → `notify_agent` (`_node_notify_agent` `:1063-1088`, writes a `Notification` row = the committed "internal-note/structured comment" equivalent) → `end`.
- **Publish gate** `POST /{id}/publish` → `_operational_validate` (`workflows_routes.py:502-639`) 409s on any critical issue. Hard requirements discovered: a node `type=="end"` must exist (`MISSING_FINAL_NODE` `:574`); every edge must reference real nodes (`BROKEN_EDGE` `:578`); anti-spam safety rules must be on — `_default_safety_rules` (`:292-296`) defaults **all-true when `definition.ops.safety_rules` is unset**, so the definition deliberately sets **no `ops` block**; `{{mustache}}` tokens in any node config surface as `MISSING_VARIABLE` (`_default_variables` `:330-355`) so the note body is **plain text, no `{{}}`**. `assign_agent`/`notify_agent` have no critical-rule (only `message`/`template_message`/`condition` do).
- The note text mirrors `docs/Prompt master.txt:197-201` (`#HANDOFF ESTRUCTURADO`: "Before assigning to @Francisco … ALWAYS add internal comment: Resumen …"). `notify_agent` `role` targeting: `VALID_ROLES={operator,tenant_admin,superadmin}` (`engine.py:122`); the tenant's **only** `TenantUser` is `role=superadmin` (verified live) — `_resolve_notify_targets` (`engine.py:1116-1135`) returns `[]` for a role with zero users and the node would silently no-op **without** recording a `WorkflowActionRun`. Targeted `role:"superadmin"` so the note is actually created (a real gotcha — recorded as smell #15).

### The exact committed trigger mechanism (file:line) — and finding #14
The **only** committed emitter of `EventType.STAGE_ENTERED` with payload `{"to": <stage>}` is the runner: `core/atendia/runner/conversation_runner.py:606-611` (inside `run_turn`, only on a real stage transition). It is consumed by **`poll_workflow_triggers`** (`core/atendia/queue/workflow_jobs.py:38-104`) — an arq cron on the dedicated `workflows` queue (`core/atendia/queue/worker.py:367-374`; `cron(... second={5,15,25,35,45,55})` ≈ every 10s) run by the **`atendia_workflow_worker`** container. That cron sweeps each tenant's `EventRow` backlog past a `WorkflowEventCursor`, calls `evaluate_event` (`engine.py:550-640`, which inserts a `WorkflowExecution` per matching **active** workflow under a savepoint with a `(workflow_id, trigger_event_id)` idempotency unique index), then runs `execute_workflow` **inline in worker context** (`workflow_jobs.py:93`). So **committing the `STAGE_ENTERED` event is the entire trigger** — no manual enqueue. `_persist_inbound`'s inline `evaluate_event` (`meta_routes.py:323`) only runs on the `MESSAGE_RECEIVED` event (emitted *before* `run_turn`), so it never sees the runner's later `STAGE_ENTERED`; the cron is the authoritative path (its own docstring `:41-44` confirms "today this cron *is* the only trigger path").

**Finding #14 — CONFIRMED (operator-action gap).** `PATCH /api/v1/conversations/{id}` with `current_stage` (the operator-realistic "move stage" action) updates `conversations.current_stage` + `conversation_state.stage_entered_at` but emits **only** `EventType.CONVERSATION_UPDATED` (`core/atendia/api/conversations_routes.py:1237-1242`) — it does **not** emit `STAGE_ENTERED`/`STAGE_CHANGED`. So a `stage_entered` workflow **does not fire when an operator moves the stage from the UI**; only an autonomous runner transition fires it. An operator wiring "on entering papeleria_completa → assign + notify" and then manually dragging a card to that stage would see the workflow silently not run. Recorded as a real product gap. (Consequently the committed trigger here is a *real runner transition*, which is the faithful production path anyway — leads reach `papeleria_completa` via the doc/Vision flow, not a manual drag.)

### Step 1 — create + publish + read-back (live, isolated tenant `867a1047`)
- `POST /api/v1/workflows` → **HTTP 201**, `workflow_id = 8f2c534f-c98d-41da-bdbb-749edb7a86e2`, nodes `[trigger_1, assign_1, note_1, end_1]`, `trigger_type=stage_entered`, `trigger_config={"to":"papeleria_completa"}`, `active=true`.
- `POST /api/v1/workflows/8f2c534f-…/publish` → **HTTP 200**, `active=true`, `version=2` (no 409 — the definition satisfied every `_operational_validate` critical rule on the first attempt).
- `GET /api/v1/workflows/8f2c534f-…` → **HTTP 200**: `active=true`, `trigger_type=stage_entered`, `trigger_config={"to":"papeleria_completa"}`, node ids/types round-trip `[trigger,assign_agent,notify_agent,end]`, `status="healthy"`, `validation.critical_count=0`.

### Step 2 — triggered for real (committed) + execution proof (verbatim DB rows)
Seeded ONLY (committed, into existing tenant `867a1047`): 1 `customers` (`phone +5218180000066`), 1 `conversations` (`current_stage=nuevo_lead`), 1 `conversation_state`. `conversation_id=71a2a174-3010-4eff-9904-bd499c8e4c5e`, `customer_id=668a2119-185f-4671-beb8-0ec1f49a7d2c`. Then set `customer.attrs={plan_credito:"sin_comprobantes_25", DOCS_INE_FRENTE/DOCS_INE_REVERSO/DOCS_COMPROBANTE_DOMICILIO:{status:"ok"}}` (committed via ORM) and ran the **REAL `ConversationRunner`** once (`OpenAINLU`+`OpenAIComposer`), `session.commit()` after the turn.

Runner turn result (verbatim): `stage_transition = "nuevo_lead->papeleria_completa"`, `stage_after=papeleria_completa`, `flow_mode=PLAN`, `outbound=[]` (composer correctly skipped — `papeleria_completa` `pause_bot_on_enter:true`), `cost_usd=0.000088` (NLU-only, gpt-4o-mini). This is the Task-4-sub-goal-3 document path used as the real committed trigger; the runner emitted `STAGE_ENTERED{to:papeleria_completa}` and the commit persisted it.

Polled `workflow_executions` (DB) until terminal. **Execution row (verbatim):**
```
id              = 3e98668a-24ce-4896-a3e3-ab10f5f60b97
workflow_id     = 8f2c534f-c98d-41da-bdbb-749edb7a86e2
conversation_id = 71a2a174-3010-4eff-9904-bd499c8e4c5e
trigger_event_id= 6888a6cd-4937-4557-bb64-4842cd6bab7e
status          = completed
current_node_id = None
error           = None      error_code = None
steps_completed = 3
started_at      = 2026-05-16 01:49:35.279509+00:00
finished_at     = 2026-05-16 01:49:35.372957+00:00
```
Triggering event (verbatim): `type=stage_entered`, `payload={'to': 'papeleria_completa'}`, `occurred_at=2026-05-16 01:49:26.128560+00:00` — execution `started_at` is **~9s later**, exactly the ≤10s cron pickup latency (proves the autonomous `atendia_workflow_worker` cron, not the script, ran it). `steps_completed=3` = assign_agent + notify_agent + end.

**Replay proof = `workflow_action_runs` (verbatim; the real per-node audit — `_execution_replay` `workflows_routes.py:750-788` is partly synthetic so the DB rows are authoritative):**
```
node_id=assign_1  action_key=wf:3e98668a-24ce-4896-a3e3-ab10f5f60b97:assign_1  created_at=2026-05-16 01:49:35.279509+00:00
node_id=note_1    action_key=wf:3e98668a-24ce-4896-a3e3-ab10f5f60b97:note_1    created_at=2026-05-16 01:49:35.279509+00:00
```
Both side-effect nodes ran (idempotency keys per `_record_action` `engine.py:884-904`): `assign_agent` set `conversation.assigned_agent_id` to the tenant's agent; `notify_agent` wrote the structured `#HANDOFF ESTRUCTURADO` `Notification`. The UI's own endpoint **`GET /api/v1/workflows/8f2c534f-…/executions`** → **HTTP 200** returned the same execution `{status:"completed", conversation_id:71a2a174-…, current_node_id:null, error:null}`.

### Step 3 — cleanup + baseline verification (verbatim)
Deleted the workflow (FK `ondelete=CASCADE` `workflow.py:43-123` drops `workflow_executions`→`workflow_action_runs`/`steps`; defensive explicit deletes too) + the seeded conversation/customer/state/events/turn_traces.
- Residual rows (all must be 0): `workflows=0, workflow_executions=0, turn_traces=0, events=0, messages=0, conversation_state=0, conversations=0, customers=0` → **all_residual_zero=true**.
- Tenant `867a1047` after vs required baseline — **identical**: `agents=1, tenant_faqs=32, tenant_catalogs=34, tenant_pipelines_active=1, conversations=0, customers=0, workflows=0, turn_traces=0` → **tenant_at_baseline=true**. Agent/KB/pipeline untouched.

### Cost
This task had **zero LLM loops** and the only LLM was the single document-path runner turn (composer skipped by `pause_bot_on_enter`): **$0.000088** (NLU gpt-4o-mini only; metered from `TurnTrace`). The workflow nodes (`assign_agent`/`notify_agent`) are pure DB writes — $0. **Task 6 real spend ≈ $0.0001.** Prior cumulative ≈ $0.144 → **new cumulative ≈ $0.144 / $1.53** (rounding-negligible add). Well under the < $0.05 task cap.

### Findings / bugs (new this task)
- **#14 CONFIRMED (product gap)** — `PATCH /api/v1/conversations/{id}` stage move emits only `CONVERSATION_UPDATED`, never `STAGE_ENTERED`/`STAGE_CHANGED` (`conversations_routes.py:1237-1242`); a `stage_entered` workflow does not fire on an operator's manual UI stage move. Only an autonomous `ConversationRunner` transition (`conversation_runner.py:606-611`) emits the event the workflow engine matches. file:line above.
- **Smell #15 (silent no-op on empty role)** — `notify_agent` with a `role` that has zero `TenantUser`s resolves to no targets and the node returns without recording a `WorkflowActionRun` or raising (`engine.py:1116-1137` returns `[]`, `_node_notify_agent:1070-1072` early-returns before `_record_action`). An operator who configures `role:"tenant_admin"` in a tenant whose only user is `superadmin` gets a "completed" execution with the notify step **silently skipped** and no audit row — no diagnostic distinguishes "ran" from "no recipients". Worked around here by targeting the role that exists; recorded as an observability gap.
- **Confirms prior #10** — the seeded lead could only reach `papeleria_completa` via the document/attrs path; the text/NLU path would have stayed in `nuevo_lead` (not re-litigated; the document path is a valid committed trigger and the production way leads actually reach this stage).
- **Positive**: the create→publish→trigger→execute→audit chain works end-to-end on the first attempt with the correct contracts; idempotency (`(workflow_id, trigger_event_id)` unique + per-node `WorkflowActionRun` unique) and FK CASCADE cleanup are sound.

### DEFERRED sub-item (declared, NOT faked)
Browser screenshot of the Workflows UI not captured — **same infra gate as Tasks 2–5** (preview tooling requires editing `.claude/launch.json`, gated as agent self-modification; frontend is Docker-served on :5173, not preview-managed). The substantive requirement (a workflow mirroring `#HANDOFF ESTRUCTURADO` is created/published via the product's REST API, triggered by a real committed event, executes both nodes, and is auditable) is proven via the workflows REST endpoints + verbatim DB execution/action-run rows. Browser pass offered to the controller at the scorecard.
